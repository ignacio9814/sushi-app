...
function saveRemoteProduct(next: Producto) {
  if (!db) return;
  void saveMenuOverride(next).catch(() => undefined);
  void updateDoc(doc(db, "productos", next.id), payloadFromProduct(next)).catch(() => undefined);
}
